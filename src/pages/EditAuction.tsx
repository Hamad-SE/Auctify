...
const EditAuction = () => {
    ...
    const { data: auction, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (user && data.seller_id !== user.id) {
                toast({
                    title: "Unauthorized",
                    description: "You cannot edit someone else's auction",
                    variant: "destructive",
                });
                navigate("/dashboard");
                return null;
    ...
    useEffect(() => {
        if (auction) {
            ...
            form.reset({
                ...
                startDate: auction.start_date ? toLocalISOString(auction.start_date) : "",
                endDate: auction.end_date ? toLocalISOString(auction.end_date) : "",
            });
            const urls = auction.image_urls && auction.image_urls.length > 0
                ? auction.image_urls
    ...
    const onSubmit = async (values: z.infer<typeof auctionSchema>) => {
        ...
        try {
            ...
            // Ensure we have at least one image if possible
            const primaryImageUrl = finalImageUrls.length > 0
                ? finalImageUrls[0]
                : (auction?.image_url || "");
            ...
            const { error: updateError } = await supabase
                ...
                .update({
                    ...
                    // We don't want to reset current_price if bids exist, but for simplicity we keep it as is if there are bids
                    // Since editing an ongoing auction might be problematic, maybe restrict it, but let's fulfill user req
                    end_date: endDateIso,
            ...
            <main className="flex-1 py-12">
                <div className="container mx-auto px-4">
                    <div className="text-center mb-8">
                        <h1 className="text-4xl font-bold mb-4 text-primary">Edit Your Item</h1>
                    ...
                    <Card className="max-w-2xl mx-auto shadow-elegant">
                        <CardHeader>
                            <CardTitle>Edit Auction Listing</CardTitle>
                            <CardDescription>Modify the details below and save changes</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <Form {...form}>
